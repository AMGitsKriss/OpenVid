...
namespace CatalogManager.Encoder
{
    public class HandbrakeStrategy : IEncoderStrategy
    {
        public void Run(VideoEncodeQueue queueItem)
        {
            ...
            string args = $@" -i ""{queueItem.InputDirectory}"" -o ""{queueItem.OutputDirectory}"" -e {queueItem.Encoder} --encoder-preset {queueItem.RenderSpeed} -f {queueItem.VideoFormat} --optimize --all-audio -q {queueItem.Quality} {dimensionArgs}";
